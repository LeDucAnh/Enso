...
public class LDAWebVCDelegate
{
    var didSelectaWordAction : ((_ word: String)->Void)!
    var didFinishLoading : ((_ result: Bool)->Void)!
...
public enum LDAWebVCProcessStatus {
    case isLoading, isFinish
}

public class LDAWebVC: UIViewController {
    
    public  var delegate =  LDAWebVCDelegate()
 ...
 */
extension LDAWebVC: WKUIDelegate {
    
    // Add any desired WKUIDelegate methods here: https://developer.apple.com/reference/webkit/wkuidelegate
    
}

extension LDAWebVC: WKNavigationDelegate ,UIGestureRecognizerDelegate{
            ...
            var selectedWord =  self.webView.evaluateJavaScript("window.getSelection().toString()", completionHandler: { (response, error) in
              ...
              //  print(error ?? <#default value#>)
                ...
                if response as! String != nil && response as! String != ""
                {
                    ...
                    if self.delegate.didSelectaWordAction != nil
                    {

                    self.delegate.didSelectaWordAction((response as! String).removeWhitespace())
                    }
                }
            })
    ...
    public func webView(_ webView: WKWebView, didStartProvisionalNavigation navigation: WKNavigation!) {
        if self.delegate.didStartLoading != nil
        {
    ...
    public func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        if self.delegate.didFinishLoading != nil
        {

        self.delegate.didFinishLoading(true)
        }
        self.networkProcess(status: .isFinish)
    ...
    public func webView(_ webView: WKWebView, didFail navigation: WKNavigation!, withError error: Error) {
        ...
        self.delegate.didFinishLoading(false)
        }
        self.networkProcess(status: .isFinish)
        updateToolbarItems()
    }
