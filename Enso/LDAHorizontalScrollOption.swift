...
@IBDesignable class LDAHorizontalScrollOptionCell:UIView
{
    ...
    var selectionViewWidthSize:CGFloat = 24.0
    func setup()
    {
        self.titleLabel.textAlignment = .center
        self.titleLabel.numberOfLines = 0
        self.selectionView.frame = CGRect(x: self.frame.size.width/2 - selectionViewWidthSize/2, y: self.frame.size.height - 4, width: selectionViewWidthSize, height: 0)
    ...
    func setSelected()
    {
        if self.state == .nonselected
        {
            ...
            selectedWidthSize = selectedWidth
            ...
            self.selectionView.frame.size.height = 4
            

            UIView.animate(withDuration: 0.5, animations: {
                self.selectionView.frame.origin.y  = self.frame.size.height - 4
    ...
    func setDeselected()
    {
        if self.state == .selected
        {
            ...
            UIView.animate(withDuration: 0.5, animations: {
                self.selectionView.frame.size.height = 0
                ...
                self.selectionView.frame.origin.y = self.frame.size.height
    ...
    func updateCellToWidth(width :CGFloat)
    {
        ...
        self.selectionView.frame.size.width = selectionViewWidthSize
        self.selectionView.frame.origin.x = self.frame.size.width/2 - selectionViewWidthSize/2
...
@IBDesignable class LDAHorizontalScrollOptionView: UIScrollView,UIScrollViewDelegate,LDAHorizontalScrollOptionCellDelegate {
    ...
    func centerScrollview(_ selectedCell:LDAHorizontalScrollOptionCell)
    {
      ...
      //      var xValue =  self.itemSize.width * CGFloat(selectedCell.tag) - CGFloat(self.displayItem/2) * (self.itemSize.width)
            var xValue =  self.itemSize.width * CGFloat(selectedCell.tag)
    ...
    func setup(titleValues: [String] ,withHeight:CGFloat = 60, displayItem:Int = 3)
    {
        ...
        let itemWidth = self.frame.size.width/CGFloat(displayItem)
